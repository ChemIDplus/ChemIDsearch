...
export class TypeElementsComponent implements OnInit, OnChanges {
	...
	ngOnChanges() :void {
		...
		const elements :Element[] = isLocator ? this.typeElements.elements.map( (element :Element) => new ResourceWithURL((<Resource>element), this.summary) ) : _.filter(this.typeElements.elements, () => true);
